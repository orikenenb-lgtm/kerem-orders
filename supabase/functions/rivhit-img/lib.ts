...
export const LIMITS = {
  ...
  preRotateCapPx: 1024,
  /** Concurrent UPSTREAM FETCHES per worker. The gate is taken BEFORE fetch()
   *  and body buffering, so peak fetch memory is bounded at
   *  maxConcurrentFetches × maxSourceBytes (4 × 8 MB = 32 MB) however many
   *  distinct variants arrive at once — the earlier design let N concurrent
   *  large bodies all buffer before the decode gate could reject them. */
  maxConcurrentFetches: 4,
  /** Concurrent decodes per worker; beyond this we fail fast instead of
   *  exhausting the worker (the measured 546 failure mode). */
  maxConcurrentDecodes: 2,
  /** Hard bound on DISTINCT in-flight variants (the dedup map size) so a
   *  flood of unique cache-buster values cannot grow the map without bound. */
  maxInflightVariants: 64,
  /** Formats ImageScript can safely decode+transform here. Anything else
   *  (gif/webp) is served as an untouched passthrough — never fed to the
   *  decoder. */
  transformableFormats: ["jpeg", "png"],
...
/** FIXED code → public reason map. The response body is built ONLY from this
 *  table — never from an upstream string, a URL, a query value, an exception
 *  message or a Content-Type. That guarantees no attacker- or upstream-
 *  controlled text can ever be reflected to the client. Internal reasons
 *  (e.g. `upstream returned 546`) are for server logs only. */
export const PUBLIC_ERROR_REASON: Record<ProxyErrorCode, string> = {
  invalid_request: "invalid request parameters",
  unsupported_source: "source not allowed",
  upstream_timeout: "source timed out",
  upstream_status: "source unavailable",
  upstream_not_image: "source is not an image",
  source_too_large: "source too large",
  invalid_dimensions: "image dimensions out of range",
  decode_failure: "could not process image",
  resource_limit: "temporarily busy, retry shortly",
  internal_error: "internal error",
};

export function errorBody(code: ProxyErrorCode): string {
  // Sanitized by construction: the reason comes from the fixed table above,
  // so it can carry no URL, stack, upstream HTML or internal path.
  return JSON.stringify({ error: code, reason: PUBLIC_ERROR_REASON[code] });
}

/** JPEG/PNG can be decoded and transformed; anything else is passed through
 *  untouched (never handed to the decoder). */
export function isTransformableFormat(format: string): boolean {
  return (LIMITS.transformableFormats as readonly string[]).includes(format);
}
