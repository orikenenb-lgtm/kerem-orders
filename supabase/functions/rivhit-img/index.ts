...
// (v11) was exported read-only and is kept in the owner's PRIVATE off-Git
// archive (never committed to this public repo). The transform pipeline below
// mirrors its decode→shrink→rotate→composite→fit order exactly, and the
// validation/limits/caching/concurrency layers are the hardening this PR adds
// on top. DO NOT deploy without the runbook in docs/edge-functions-runbook.md
// and owner approval.
//
...
//   * FETCH gate before buffering + decode gate + global in-flight-variant
//     bound, all fast-reject when saturated (bounds peak memory)
//   * explicit format handling: only JPEG/PNG decoded; GIF/WebP passthrough
//   * in-flight dedup per variant
//   * short negative cache on errors so a dead link cannot re-hammer the worker
//   * error bodies built ONLY from a fixed code→reason table — no upstream
//     string, URL, query value or stack can ever be reflected; privacy-safe logs
...
// Fetch gate: acquired BEFORE fetch() and body buffering, so N concurrent
// large bodies can never all sit in memory at once (the earlier design only
// gated the decode, after buffering). Fast-reject when saturated — the client
// absorbs a resource_limit via its retry/direct fallback. Slot released in
// finally, always.
async function guardedFetch(
  ...
  if (activeFetches >= LIMITS.maxConcurrentFetches) {
    return { ok: false, code: "resource_limit", reason: "too many concurrent fetches" };
  }
  activeFetches++;
  try {
    return await fetchUpstream(url, maxBytes, truncateAtCap);
  } finally {
    activeFetches--;
  }
}

// The `internalReason` is accepted for readability at call sites and for
// server-side logging ONLY — it is NEVER placed in the response body, which
// is built solely from the fixed PUBLIC_ERROR_REASON table inside errorBody.
// This is what guarantees no upstream string / URL / stack can be reflected.
function errorBaked(code: ProxyErrorCode, _internalReason?: string): Baked {
  return {
    status: statusForError(code),
    headers: errorHeaders(),
    body: new TextEncoder().encode(errorBody(code)),
  };
}
...
async function process(req: ProxyRequest): Promise<Baked> {
  ...
  if (req.meta) {
    const fetched = await guardedFetch(req.url, LIMITS.metaReadBytes, true);
    if (!fetched.ok) {
  ...
  const fetched = await guardedFetch(req.url, LIMITS.maxSourceBytes);
  if (!fetched.ok) {
  ...
  // Explicit format handling: only JPEG/PNG are fed to the decoder. A GIF or
  // WebP that needs a transform is served as its untouched original instead
  // of being pushed into a decoder that may mishandle it — the sniffed
  // content-type is correct, and the client shows a real (if unresized)
  // image rather than a placeholder.
  if (!isTransformableFormat(sniffed.format)) {
    log("ok", "passthrough-format", { fmt: sniffed.format, bytes: fetched.bytes.byteLength });
    return { status: 200, headers: successHeaders(`image/${sniffed.format}`), body: fetched.bytes };
  }
...
Deno.serve(async (request: Request): Promise<Response> => {
  ...
  try {
    ...
    if (!pending) {
      // Global bound on DISTINCT in-flight variants: a flood of unique
      // cache-buster values cannot grow the dedup map without limit. An
      // already-in-flight variant (found above) is always allowed to join.
      if (inflight.size >= LIMITS.maxInflightVariants) {
        logEvent({ outcome: "error", stage: "inflight-cap", code: "resource_limit", size: inflight.size });
        const b = errorBaked("resource_limit", "too many in-flight variants");
        return new Response(b.body, { status: b.status, headers: b.headers });
      }
