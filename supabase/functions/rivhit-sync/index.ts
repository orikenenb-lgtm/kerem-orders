...
// product was still written, with category '' — and since the catalogue's
// category lists are built from active products' categories, one bad
// Rivhit response would have emptied every category on the site for 15
// minutes and shown "no category" everywhere. Now the run aborts (status
// 'error', nothing written) and the next run, 15 minutes later, retries.
// Everything else is byte-for-byte v19.
  ...
  const work = (async () => {
    ...
    try {
      ...
      if (what === 'products' || what === 'both') {
        ...
        // Group names ARE the site's categories. A failed or empty answer here
        // must abort the run, not fall through: writing 900 products with an
        // empty category would blank every category on the site until the
        // next run. (The audit function has refused an empty group list since
        // day one; the sync now holds itself to the same rule.)
        const g = await rivhit('Item.Groups')
        const gl: any[] = (g?.data && g.data.item_group_list) || []
        if (g?.error_code !== 0 || gl.length === 0) {
          throw new Error(`Item.Groups error_code ${g?.error_code}: ${g?.client_message || 'empty group list'} — nothing written, will retry next run`)
        }
        for (const x of gl) {
