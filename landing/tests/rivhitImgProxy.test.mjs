...
const {
  ...
  parseExifOrientation, exifOrientationToDegrees, normalizeRotation, exceedsPixelCap, preRotateCapFor,
  statusForError, cacheKeyFor, successHeaders, errorHeaders, errorBody,
  PUBLIC_ERROR_REASON, isTransformableFormat,
...
t("shrink-before-rotate cap matches the deployed v11 rule exactly", () => {
  assert.equal(LIMITS.preRotateCapPx, 1024);         // deployed HARD_CAP_PX
  assert.equal(preRotateCapFor(0), 1024);            // w=0: original, capped
  assert.equal(preRotateCapFor(480), 480);           // grid width — no upscale
  assert.equal(preRotateCapFor(100), 320);           // deployed floor of 320
  assert.equal(preRotateCapFor(1600), 1024);         // never above the cap
});
...
t("error bodies come ONLY from the fixed table — no upstream string can leak", () => {
  // errorBody takes just the code; the reason is always the fixed public one.
  const b = JSON.parse(errorBody("decode_failure"));
  assert.deepEqual(Object.keys(b).sort(), ["error", "reason"]);
  assert.equal(b.reason, PUBLIC_ERROR_REASON.decode_failure);
  // Every code in the taxonomy has a fixed, safe reason and none of them can
  // carry a URL, a token, a path or a stack.
  const HOSTILE = /https?:\/\/|api\.rivhit|token|Bearer|\/online\/|Error:|at \w|\.ts:|stack/i;
  for (const [code, reason] of Object.entries(PUBLIC_ERROR_REASON)) {
    assert.equal(typeof reason, "string");
    assert.ok(reason.length > 0 && reason.length < 60, code);
    assert.ok(!HOSTILE.test(reason), `public reason for ${code} must not leak internals`);
    // The serialized body for this code likewise carries nothing hostile.
    assert.ok(!HOSTILE.test(errorBody(code)), `error body for ${code} must be clean`);
  }
});
t("only JPEG and PNG are transformable; GIF/WebP are passthrough-only", () => {
  assert.equal(isTransformableFormat("jpeg"), true);
  assert.equal(isTransformableFormat("png"), true);
  assert.equal(isTransformableFormat("gif"), false);
  assert.equal(isTransformableFormat("webp"), false);
  assert.equal(isTransformableFormat("svg"), false);
});
t("concurrency + inflight bounds are set to memory-safe values", () => {
  assert.ok(LIMITS.maxConcurrentFetches >= 1 && LIMITS.maxConcurrentFetches <= 8);
  assert.ok(LIMITS.maxConcurrentDecodes >= 1 && LIMITS.maxConcurrentDecodes <= LIMITS.maxConcurrentFetches);
  // Peak buffered fetch memory must stay well under a typical worker budget.
  assert.ok(LIMITS.maxConcurrentFetches * LIMITS.maxSourceBytes <= 64 * 1024 * 1024);
  assert.ok(LIMITS.maxInflightVariants >= 8 && LIMITS.maxInflightVariants <= 256);
});
